...
def parse(bam, chroms):
    ...
    #     print(k,v)
    # 1/0
    header = "\t".join(['query', 'chrom', 'mapped_len', 'chrom_len', 'cov_perc', 'assignment'])
    print(header)
    for chrom, vals in res.items():
        for idx, v in enumerate(vals):
            contig, chrom, mapped, ref_len, cov = v

            new_name = chrom if idx == 0 else chrom + f"_unloc{idx}"

            res = "\t".join([contig, chrom, str(mapped), str(ref_len), str(cov), new_name])
            print(res)
